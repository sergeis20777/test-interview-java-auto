...
- **Метод**: `testLoginWithWrongPassword`
- **Пользователь**: `standard_user`
- **Пароль**: `wrong_password` (неверный)
- **Проверка**: Отображается сообщение об ошибке (текст содержит «do not match»), пользователь остаётся на странице логина

### Тест 3: Логин заблокированного пользователя (locked_out_user)
- **Метод**: `testLoginLockedOutUser`
- **Пользователь**: `locked_out_user`
- **Пароль**: `secret_sauce`
- **Проверка**: Отображается сообщение об ошибке (текст содержит «locked out»), пользователь остаётся на странице логина

### Тест 4: Логин с пустыми полями
- **Метод**: `testLoginWithEmptyFields`
- **Пользователь**: — (поле пустое)
- **Пароль**: — (поле пустое)
- **Проверка**: Отображается сообщение об ошибке (текст содержит «required», например «Username is required»), пользователь остаётся на странице логина

### Тест 5: Логин пользователем performance_glitch_user
- **Метод**: `testLoginPerformanceGlitchUser`
- **Пользователь**: `performance_glitch_user`
- **Пароль**: `secret_sauce`
- **Проверка**: Корректный переход на страницу продуктов; страница открывается несмотря на имитируемые задержки (используются явные ожидания WebDriverWait)